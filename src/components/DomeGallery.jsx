...
function buildItems(pool, seg) {
    ...
    const coords = xCols.flatMap((x, c) => {
        ...
        return ys.map(y => ({ x, y, sizeX: 2.2, sizeY: 2.2 }));
    });
...
export default function DomeGallery({
    ...
    const onTileClick = useCallback(
        e => {
            ...
            if (openingRef.current) return;
            if (onTileSelect) {
                const idx = parseInt(e.currentTarget.parentElement?.dataset?.index ?? '-1', 10);
                const pool = images;
                if (idx >= 0 && idx < pool.length) {
                    const img = typeof pool[idx] === 'string' ? { src: pool[idx], alt: '' } : pool[idx];
                    onTileSelect(img);
                }
                return;
            }
        ...
        [openItemFromElement, onTileSelect, images]
    );
                ...
                <div className="stage">
                    <div ref={sphereRef} className="sphere">
                        {items.map((it, i) => {
                            ...
                            const poolIdx = poolLen > 0 ? i % poolLen : 0;
                            return (
                                <div
                                    key={`${it.x},${it.y},${i}`}
                                    className="item"
