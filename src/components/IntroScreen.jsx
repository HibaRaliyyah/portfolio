...
export function IntroScreen() {
    ...
    // Responsive state — drives conditional rendering of desktop-only elements
    ...
    // Scale the 1440×900 design canvas to fit the viewport
    useEffect(() => {
        ...
        const apply = () => {
            ...
            // Tablet: 0.82 keeps text readable while fitting the viewport
            // Mobile: 0.92 squeezes a bit more to avoid vertical overflow
            const factor = vw <= 768 ? 0.92 : vw <= 1024 ? 0.82 : 1;
    ...
    return (
        <AnimatePresence>
            {phase === 'intro' && (
                ...
                >
                    {/* Background fills the ENTIRE viewport at all sizes */}
                    <img
                        src={portfolioHero}
                        draggable={false}
                    ...
                    {/* Content layer — scales the 1440×900 design to fit while
                         keeping it full-bleed on matching-aspect screens. The
                         background image behind always covers the viewport, so the
                         intro never appears small with empty margins. */}
                    <div
                        ref={scaleRef}
                        className="absolute inset-0 pointer-events-none intro-scale-wrapper"
                    >
                        <div
                            className="relative"
                            style={{
                        ...
                        >
                            ...
                            <motion.p
                                ...
                                className="text-[#FFF] font-islandMoments text-[230px] leading-[120px] w-[360px] h-[210px] absolute left-[120px] top-[360px] text-center drop-shadow-[0_4px_24px_rgba(0,0,0,0.85)] pointer-events-none"
                            ...
                            <motion.div
                                ...
                                className="absolute left-[60px] top-[670px] z-10 flex items-center justify-center select-none"
                                style={{
                                    width: '670px',
                                    height: '200px',
                                }}
                            ...
                            {/* ─── CSE & Final Year Badge (hidden on mobile) ─── */}
                            {!isMobile && (
                                <motion.div
                                    initial={{ opacity: 0, x: 40 }}
                                    animate={{ opacity: 1, x: 0 }}
                                    transition={{ duration: 0.8, delay: 0.4 }}
                                    className="absolute left-[1060px] top-[400px] z-10 select-none flex flex-col items-center justify-center"
                                    style={{ width: '1080px', height: '230px' }}
                                >
                                    ...
                                    {/* CSE Text */}
                                    <p
                                        className="relative z-10 text-[#17430B] font-luxuriousRoman text-[80px] leading-[86px] text-center font-bold translate-y-[-38px]"
                                    ...
                                    {/* Final Year Text */}
                                    <p
                                        className="relative z-10 text-[#FFF] font-poppins text-[70px] leading-[50px] text-center font-semibold drop-shadow-md translate-y-[-28px]"
                            ...
                            {/*
                              Positioning: CSS handles left/top for responsive layout.
                              Framer Motion animate only controls scale + boxShadow
                              (the infinite breathing pulse), so it won't override
                              the CSS positioning.
                            */}
                            <motion.button
                                type="button"
                                ...
                                className={`
                                    ...
                                    w-[320px] h-[80px] rounded-[50px]
                                    bg-[#20A619] border border-white/30
                                    flex items-center justify-center
                                    shadow-[0_0_20px_rgba(32,166,25,0.6)]
                                    hover:shadow-[0_0_35px_rgba(32,166,25,0.9)]
                                    transition-shadow
                                    ${isMobile
                                        ? 'bottom-13 right-24'
                                        : isTablet
                                            ? 'bottom-10 right-6'
                                            : 'bottom-[130px] right-[-350px]'
                                    }
                                `}
                            >
                                <span
                                    className="text-[#FFF] font-pressStart2P text-md leading-none text-center flex items-center justify-center gap-1"
                            ...
                            {!isMobile && (
                                <div className="absolute bottom-8 right-24 flex items-center gap-[107px] z-20">
                                    ...
                                    <motion.a
                                        ...
                                        className="w-[88px] h-[88px] flex items-center justify-center rounded-full bg-white/25 hover:bg-white/30 border border-white/25 backdrop-blur-md transition-colors cursor-pointer group shadow-lg"
                                    >
                                        <svg className="w-[44px] h-[44px] text-white fill-current group-hover:text-white transition-colors" viewBox="0 0 24 24">
                                    ...
                                    >
                                        <svg
                                            ...
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                            className="w-[44px] h-[44px] text-white group-hover:text-white transition-colors"
                                        >
                                            <path
                                                fillRule="evenodd"
                                                clipRule="evenodd"
                                    ...
                                    >
                                        <img
                                            src={hibiIcon}
                                            draggable={false}
                                            className="w-[48px] h-[48px] object-contain max-w-none grayscale"
                                            alt="Hibi assistant"
                                        />
                                    </motion.a>
                                </div>
